...
function CreateListing() {
  ...
  const handleImageSubmit=(e)=>{
    if(files.length>0 && files.length+formData.imageUrls.length<7){
      ...
      Promise.all(promises).then((urls)=>{
        //formData.imageUrls.concat(urls) is used to add the urls to existing imageUrl array
  ...
  const handleRemoveImage = (index) => {
    setFormData({
      ...formData,
      //Filter method will delete the url that we want to delete
       ...
       <form className='flex flex-col sm:flex-row gap-6'>
          ...
          <span className='font-normal text-gray-600 ml-2'>The first image will be the cover(max 6)</span>
           ...
           <div className='flex gap-4'>
               ...
               rounded uppercase hover:shadow-lg disabled:opacity-85'>{uploading? '...Uploading':'Upload'}</button>
            ...
            <p className='text-red-700'>{imageUploadError && imageUploadError}</p>
            {
              formData.imageUrls.length>0 && formData.imageUrls.map((url,index)=>(
                <div key={url} className="flex justify-between p-3 border items-center">
                  ...
                  className='uppercase p-3 text-red-700 rounded-lg hover:opacity:75'>Delete
                 </button>
                  {/* The callback function is called in the above button as if it is not done the image will
                  be deleted without clicking on the image button */}
                </div>
              ))
            }
